...
func (p *Parser) expr(rbp int) ast.Expression {
	...
	for p.currentToken.Precedence() > rbp {
		...
		if p.currentToken.IsBinary() && p.currentToken.IsLeftAssociative() {
			...
			default:
				p.expect("binary operator", p.currentLexme)
			}
		} else {
			break
		}
...
func (p *Parser) unary(t epllex.Token) ast.Expression {
...
// boolean expression version of unary
