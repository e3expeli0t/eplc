...
type Token struct {
	Lexme string
	Ttype TokenType
	Start [2]uint //index 1 is line index 2 is ofsset 
}
...
func resolveType(buffer bytes.Buffer, start [2]uint) Token {
	reserve()
	...
	return Token{Ttype: ID, Lexme: buffer.String(), Start: start}
}
